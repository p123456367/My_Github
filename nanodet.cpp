...
void NanoDet::decode_infer(ncnn::Mat& feats, std::vector<CenterPrior>& center_priors, float threshold, std::vector<std::vector<BoxInfo>>& results)
{
    ...
    // 创建一个新的 Mat 用于存储 WHC 格式的数据
    ncnn::Mat new_mat(h, w, c);

    // 遍历并重新排列数据
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            for (int channel = 0; channel < c; channel++) {
                // HWC 格式 (height, width, channel) -> WHC 格式 (width, height, channel)
                new_mat.channel(channel).row(x)[y] = (float)feats.channel(channel).row(y)[x];
            }
        }
    }
    ...
    for (int idx = 0; idx < num_points; idx++)
    {
        ...
        /*printf("%d\n", 2);*/
        //ncnn::Mat feats = feats.channel(idx);
       /* const float* scores = feats.row(0);*/
        const float* scores = new_mat.row(idx);
       /* printf("%d\n", 3);*/
        float score = 0;
        int cur_label = 0;
        ...
        if (score > threshold)
        {
            ...
            //ncnn::Mat feats = feats.channel(idx);
            const float* bbox_pred = new_mat.row(idx) + this->num_class;
